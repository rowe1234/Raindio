...
// =============================================================================
// Water Wave System (使用 noisetex 硬件采样优化)
// =============================================================================
...
// 使用纹理采样替代原本高开销的哈希数学运算
float noise(vec2 p) {
    return texture2D(noisetex, p * 0.00390625).r * 2.0 - 1.0; // 1.0 / 256.0
}
...
const float rcp_total_height = 2.37898; // 预计算 1.0 / total_height 常数
...
vec4 ray_trace_ssr(vec3 direction, vec3 start) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        vec2 uv = ViewToScreen(testPoint);
        ...
        if (rawDepth < 0.9999) {
            ...
            if (diff > -0.0001 && diff < 1.5) {
                ...
                if (waterFlag > 0.9) {
                    ...
                    return vec4(hitColor, edge.x * edge.y);
                }
            }
            float contract = 1.0 - 0.03125 * float(i + 1); // 0.03125 = 1/32
...
void main() {
    ...
    if (isWater) {
        ...
        if (underwater) {
            rawWorldNormal.y = -rawWorldNormal.y;
        }
        ...
        if (ssr.a < 0.95) {
            if (!underwater) {
                ...
                if (realSunDir.y > -0.05) {
                    ...
                    skyRefl = AtmosphericScattering(reflectWorldDir, -realSunDir, 0.1) * 0.15 + vec3(0.002, 0.004, 0.01);
                }
                
                skyRefl = max(skyRefl, vec3(0.005, 0.01, 0.03)) * horizonFade;
                reflection = mix(skyRefl, ssr.rgb, ssr.a);
            } else {
                reflection = mix(vec3(0.005, 0.03, 0.07), ssr.rgb, ssr.a);
            }
        }
        ...
        float fresnel = 0.02 + 0.98 * pow(max(1.0 - NdotV, 0.0), 5.0);
        ...
        if (underwater) {
            ...
            // roughness = 0.1, a2 = 0.0001, a2 - 1.0 = -0.9999
            float denom = NdotH * NdotH * -0.9999 + 1.0;
            float D = 0.0001 / (3.14159265 * denom * denom);
    ...
    float rawLum = max(texture2D(colortex6, vec2(0.5)).r, 0.0001);
    ...
    if (realSunDir.y > -0.05) {
        ...
        skyHDR = AtmosphericScattering(worldDir, -realSunDir, 0.05) * 0.02 + vec3(0.001, 0.002, 0.005);
    }
    ...
    sunDiscMask = pow(max(sunDiscMask * sunDiscMask * (3.0 - 2.0 * sunDiscMask), 0.0), 2.0);
    ...
    if (realSunDir.y < 0.0) skyHDR += SunDisc(worldDir, -realSunDir) * vec3(0.7, 0.75, 0.9) * 0.35;
    ...
    // 数学简化：(cn^(1/0.95)) / cn -> cn^0.0526316
