...
// 预计算 Vogel Disk 采样方向 (保留原采样点)
const vec2 VOGEL_SAMPLES[14] = vec2[](
...
vec3 extractPureLight(vec3 color, float threshold) {
    ...
    if (lum <= 0.0001) return vec3(0.0);
    
    // 1.0 / (4.0 * 0.1 + 0.0001) = 2.49937516
...
void makeDualBloom(float threshold, vec2 uv, vec2 texel, out vec3 tight, out vec3 wide) {
    ...
    for (int i = 0; i < 14; i++) {
        ...
        if (all(greaterThanEqual(uvTight, vec2(0.0))) && all(lessThanEqual(uvTight, vec2(1.0)))) {
            sumTight += extractPureLight(texture2D(colortex0, uvTight).rgb, threshold) * w;
            wTightSum += w;
        }
        if (all(greaterThanEqual(uvWide, vec2(0.0))) && all(lessThanEqual(uvWide, vec2(1.0)))) {
            sumWide += extractPureLight(texture2D(colortex0, uvWide).rgb, threshold) * w;
            wWideSum += w;
        }
    }
...
vec3 makeCrossGlareOptimized(float glarePixels, vec2 uv, float threshold, vec2 texel) {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        vec3 c0 = extractPureLight(texture2D(colortex0, uv0).rgb, threshold);
        vec3 c1 = extractPureLight(texture2D(colortex0, uv1).rgb, threshold);
...
void main() {
    ...
    gl_FragData[1] = vec4(outTight, 1.0);
    gl_FragData[2] = vec4(outWide, 1.0);
}